...
class AdvancedBacktester:
    ...
    def backtest_with_positions(
        ...
        *,
        ...
        max_long: float = 1.0,
        max_short: float = 1.0,
        apply_position_lag: bool = True,  # Nuclear Redesign: Fix look-ahead bias
    ) -> BacktestResults:
        """
        Run backtest with given position sizing.

        Nuclear Redesign: Added `apply_position_lag` parameter to fix look-ahead bias.

        The Problem:
        - Features from day T are used to generate a prediction for day T+1
        - The prediction determines position sizing for day T+1
        - But previously, position[T] was applied to returns[T] (same day!)
        - This creates look-ahead bias: we're using day T's prediction on day T's return

        The Fix:
        - When apply_position_lag=True (default), shift positions by 1 day
        - position[T] is applied to returns[T+1] (next day's return)
        - This correctly simulates: predict on day T, execute on day T+1

        Args:
            apply_position_lag: If True (default), apply 1-day lag to positions.
                                Set to False for backward compatibility with old backtests.
        """
        dates = np.asarray(dates)
        prices = np.asarray(prices)
        returns = np.asarray(returns)
        ...
        # ========================================================================
        # NUCLEAR REDESIGN: Apply 1-day position lag to fix look-ahead bias
        # ========================================================================
        # positions[T] now applies to returns[T+1], not returns[T]
        # This correctly models: signal on day T → trade on day T+1
        # ========================================================================
        if apply_position_lag:
            ...
            positions_lagged[0] = 0.0  # No position on first day (no prior signal)
        ...
        # Calculate turnover: average absolute position change per period
        # Turnover measures how frequently the portfolio is rebalanced
    ...
    def _compute_metrics(self, daily_returns: np.ndarray) -> Dict[str, float]:
        ...
        # TURNOVER: Average absolute position change per period
        # This will be computed from trade log in backtest_with_positions()
        # For now, set to 0 as placeholder (will be overridden)
        turnover = 0.0
