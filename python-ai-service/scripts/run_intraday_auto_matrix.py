"""Run intraday matrix using automatic variant routing (no retraining)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Intraday matrix with auto variant routing")
    ...
    out_path = out_dir / f"intraday_auto_matrix_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.csv"
